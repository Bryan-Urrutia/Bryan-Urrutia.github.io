...
export default function Dibujo() {
	...
	const drawHeart = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string) => {
		...
		ctx.beginPath();
		ctx.moveTo(x, y - s / 4);
		ctx.bezierCurveTo(x - s, y - s, x - s * 1.5, y + s / 3, x, y + s);
		ctx.bezierCurveTo(x + s * 1.5, y + s / 3, x + s, y - s, x, y - s / 4);
		ctx.closePath();
		ctx.fillStyle = color;
		ctx.fill();
	};
	...
	const drawFlower = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string) => {
		...
		for (let i = 0; i < 8; i++) {
			...
			ctx.beginPath();
			ctx.arc(x + Math.cos(angle) * size / 2, y + Math.sin(angle) * size / 2, size / 3, 0, Math.PI * 2);
			ctx.fill();
		}
		ctx.beginPath();
	...
	const drawButterfly = (ctx: CanvasRenderingContext2D, x: number, y: number, size: number, color: string) => {
		ctx.fillStyle = color;
		ctx.beginPath();
		const w = size;
		const h = size;
		ctx.moveTo(x, y);
		ctx.bezierCurveTo(x - w * 0.6, y - h * 0.6, x - w, y - h * 0.1, x - w * 0.4, y + h * 0.1);
		ctx.bezierCurveTo(x - w * 0.9, y + h * 0.6, x - w * 0.2, y + h * 0.7, x, y + h * 0.2);
		ctx.bezierCurveTo(x + w * 0.2, y + h * 0.7, x + w * 0.9, y + h * 0.6, x + w * 0.4, y + h * 0.1);
		ctx.bezierCurveTo(x + w, y - h * 0.1, x + w * 0.6, y - h * 0.6, x, y);
		ctx.fill();
	};

	const drawShape = (ctx: CanvasRenderingContext2D, x: number, y: number) => {
		if (shapeType === "star") drawStar(ctx, x, y, 20, brushColor);
		if (shapeType === "heart") drawHeart(ctx, x, y, 20, brushColor);
		if (shapeType === "flower") drawFlower(ctx, x, y, 24, brushColor);
		if (shapeType === "butterfly") drawButterfly(ctx, x, y, 34, brushColor);
		fillBackground(ctx);
	};
	...
	return (
		<div className="flex flex-col gap-4 items-center pt-10">
			...
			<canvas
				...
				width={canvasWidth}
				height={canvasHeight}
				className="border-2 border-gray-400 rounded touch-none"
